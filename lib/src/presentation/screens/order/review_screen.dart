...
class _ReviewScreenState extends State<ReviewScreen> {
  ...
  void initState() {
    super.initState();
    _db.getDocumentFromReference(widget.order.restaurant).then((value) {
      if (mounted) {
        setState(() {
          final data = value.data();
          if (data != null) {
            final map = data as Map<String, dynamic>;
            restaurant = map["name"] ?? "Unknown";
            debugPrint("Category fetched: $restaurant");
          } else {
            restaurant = "Unknown";
            debugPrint("No data found for restaurant reference");
          }
        });
      }
    }).catchError((error) {
      debugPrint("Error fetching restaurant: $error");
    });
  }
                  ...
                  child: Center(
                    child: Column(
                      children: [
                        ...
                        const SizedBox(height: 20),
                        Text(
                          "Tolong tinggalkan ulasan untuk",
                          textAlign: TextAlign.center,
                          style: CustomTextStyle.size14Weight400Text(
                            AppColors().secondaryTextColor,
                          ),
